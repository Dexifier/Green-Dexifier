...
// ...existing code...
export default function SwapPage({ params }: PageProps) {
  ...
  useEffect(() => {
    if (chains.length && coins.length && !init) {
      const match = slug.match(/^swap-([^-]+)-([^-]+)-to-([^-]+)-([^-]+)$/);
      if (match) {
        const [, coinFrom, chainFrom, coinTo, chainTo] = match;
        setInit(true);
        initToken(
          { chain: chainFrom.toUpperCase(), coin: coinFrom.toUpperCase() },
          { chain: chainTo.toUpperCase(), coin: coinTo.toUpperCase(), }
        );
      }
    }
  }, [slug, initToken, chains, coins, init]);
