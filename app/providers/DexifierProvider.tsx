...
import {
  createQuotes,
  createSwap as createChainflipSwap,
  getSwapStatus,
} from '../api/chainflip';
...
interface DexifierContextType {
  ...
  setSwapData: Dispatch<
    SetStateAction<
      ChainflipSwapResponse | ExTxInfo | ConfirmRouteResponse | undefined
    >
  >;
  swapStatus?: ChainflipSwapStatus | ExTxInfo;
  setSwapStatus: Dispatch<
    SetStateAction<ChainflipSwapStatus | ExTxInfo | undefined>
  >;
  initialize: () => void;
  stopConfirming: () => void;
...
const DexifierContext: React.Context<DexifierContextType | undefined> =
  createContext<DexifierContextType | undefined>(undefined);
...
const DexifierProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [swapData, setSwapData] = useState<
    ChainflipSwapResponse | ExTxInfo | ConfirmRouteResponse
  >();
  const [swapStatus, setSwapStatus] = useState<
    ChainflipSwapStatus | ExTxInfo
  >();
  ...
  const chains: Blockchain[] = useMemo(() => {
    if (blockchains.length && networks.length) {
      ...
      const nt = networks
        ...
        .filter(
          (network) =>
            !bc.some(
              (blockchain) =>
                blockchain.name === network.name ||
                MAP_BLOCKCHAIN_RANGO_2_EXOLIX[blockchain.name] === network.name
            )
        );
      return [...bc, ...nt];
    } else return [];
  ...
  const coins: Token[] = useMemo(() => {
    if (networks.length && tokens.length && currencies.length) {
  ...
  const createSwap = async () => {
    ...
    if ('egressAmount' in selectedRoute) {
      ...
      try {
        ...
        //   );
        const minimumPrice =
          selectedRoute.egressAmount *
          (1 - parseFloat(settings.slippage) / 100);
        ...
        if (error instanceof AxiosError) {
          throw new Error(
            (error.response?.data as ChainflipError).detail || ''
          );
        }
        throw error;
      }
  ...
  const debounceFetchRoutes = useMemo(() => {
    const getRoutes = async (
      ...
      try {
        ...
        const { data: exolixRateResponse }: { data: RateResponse } =
          await axiosExolix.get('/rate', {
      ...
      if (!isMobile)
        try {
          const rangoMultiRouteRequest: MultiRouteRequest = {
            ...
            swapperGroups: settings.swappers.map(
              (swapper) => swapper.swapperGroup
            ),
            swappersGroupsExclude: false,
          };
          const rangoMultiRouteResponse: MultiRouteResponse =
            await rangoSDK.getAllRoutes(rangoMultiRouteRequest);
          const rangoMultiRouteSimulationResults: MultiRouteSimulationResult[] =
            rangoMultiRouteResponse.results.sort(
  ...
  }, [tokenFrom, tokenTo, amountFrom, debounceFetchRoutes]);
  ...
  const sendTx = async (recipient: string) => {
    if (
      ...
      return (await getSigners(walletFrom.walletType))
        .getSigner(TransactionType.EVM)
        .signAndSendTx(transaction, walletFrom.address, null)
      ...
      if ('amountTo' in swapData) {
        confirmIntervalRef.current = setInterval(async () => {
          ...
          if (
            ...
            txInfo.status === 'overdue' ||
            txInfo.status === 'refunded'
          ) {
            stopConfirming();
          }
  ...
  useEffect(() => {
    axios.get(`/api/exolix/currency`).then((result) => {
      setCurrencies(result.data as DCurrency[]);
    });
    axios.get(`/api/exolix/network`).then((result) => {
